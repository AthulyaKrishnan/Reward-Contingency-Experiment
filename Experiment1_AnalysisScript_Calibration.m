...
%% main loop
    ...
    for part = 1:48
        if  part==33 %% excluded because of implausible reaction time of <1ms across all trials
            skipText='Skipping';
            continue;
        else 
           
        % gender_path = basic_path;
        % gender_tab = readtable(fullfile(gender_path,'CRD4_Demographics.xlsx'));
        % gender = gender_tab(part,3);
        % 
        % curr_gender =  gender{1,1}; 
        % if curr_gender == 2
